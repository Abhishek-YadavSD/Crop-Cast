...
def yield_prediction():
    ...
    metrics_html = ""  # Initialize metrics_html to an empty string
    if selected_crop:
        ...
        metrics_html = "<h3>Model Performance Metrics:</h3>"
        for metric, value in metrics.items():
            metrics_html += f"<p>{metric}: {value:.4f}</p>"
        ...
        return template('views/yield.html', selected_crop=selected_crop, yield_chart=yield_chart_filename, crop_list=crop_list, metrics_html=metrics_html)
    return template('views/yield.html', selected_crop=None, yield_chart=None, crop_list=crop_list, metrics_html=metrics_html)
...
def analysis():
    ...
    return template('analysis.html', results=analysis_results, selected_state=selected_state, states=states)
